...
namesTracking = ['neutral', 'boy_aa', 'girl_ca', 'cat']
...
while True:
    ...
    for index, name in enumerate(namesTracking):
        ...
        if confidences[index] is not None:
            confidences[index] = re.search('\d+', confidences[index][0])
    ...
    for index, confidence in enumerate(confidences):
        if confidence is not None:  # it found the confidence, so the whole line (with x,y,name,etc) exists
            if int(confidence[0]) >= priorConfidences[index]:
                ...
                #print(f'Confidence of {confidence[0]} not higher than {priorConfidences[index]}')
                pass
    ...
    if shouldSend:
        for index, priorConfidence in enumerate(priorConfidences):
            if priorConfidence >= 0:
                ...
                #print(f'Sending to {udpPorts[index]}: confidence={priorConfidence}, x={bestXs[index]}, y={bestYs[index]}, width={bestWidths[index]}, height={bestHeights[index]}')
